import {
  AreaChart,
  Area,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  ResponsiveContainer,
  Legend,
  PieChart,
  Pie,
  Cell,
} from "recharts";
import { TrendingUp, DollarSign, PiggyBank, Target } from "lucide-react";

export function FinancialSection() {
  const revenueData = [
    { year: "2024", revenue: 250, profit: 50 },
    { year: "2025", revenue: 800, profit: 200 },
    { year: "2026", revenue: 2000, profit: 600 },
    { year: "2027", revenue: 4500, profit: 1500 },
    { year: "2028", revenue: 8000, profit: 3000 },
  ];

  const costBreakdown = [
    { name: "Виробництво", value: 40, color: "#144073" },
    { name: "Маркетинг", value: 25, color: "#1A6DCC" },
    { name: "R&D", value: 15, color: "#5a9de8" },
    { name: "Операційні", value: 12, color: "#84b6f1" },
    { name: "Інше", value: 8, color: "#b3d4f5" },
  ];

  const metrics = [
    {
      icon: TrendingUp,
      label: "Прогнозований дохід 2025",
      value: "$800K",
      change: "+220%",
      positive: true,
    },
    {
      icon: DollarSign,
      label: "Середній чек",
      value: "$12K",
      change: "за установку",
      positive: true,
    },
    {
      icon: PiggyBank,
      label: "Необхідні інвестиції",
      value: "$500K",
      change: "Seed раунд",
      positive: true,
    },
    {
      icon: Target,
      label: "Break-even",
      value: "Q3 2025",
      change: "18 місяців",
      positive: true,
    },
  ];

  const unitEconomics = [
    { metric: "Середня ціна продажу", value: "$12,000" },
    { metric: "Собівартість виробництва", value: "$6,500" },
    { metric: "Валова маржа", value: "$5,500 (45%)" },
    { metric: "Вартість залучення клієнта (CAC)", value: "$200" },
    { metric: "Lifetime Value (LTV)", value: "$18,000" },
    { metric: "LTV/CAC", value: "90x" },
  ];

  return (
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl text-primary mb-4">
            Фінансова модель
          </h2>
          <p className="text-xl text-foreground/80">
            Прозора фінансова стратегія та прогнози зростання
          </p>
        </div>

        {/* Key Metrics */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {metrics.map((metric, index) => {
            const Icon = metric.icon;
            return (
              <div
                key={index}
                className="bg-muted rounded-xl p-6 hover:shadow-lg transition-all"
              >
                <div className="inline-flex items-center justify-center w-12 h-12 bg-secondary/10 rounded-lg mb-4">
                  <Icon className="text-secondary" size={24} />
                </div>
                <div className="text-sm text-foreground/60 mb-2">
                  {metric.label}
                </div>
                <div className="text-3xl text-primary mb-2">{metric.value}</div>
                <div
                  className={`text-sm ${
                    metric.positive ? "text-green-600" : "text-foreground/60"
                  }`}
                >
                  {metric.change}
                </div>
              </div>
            );
          })}
        </div>

        {/* Revenue & Profit Chart */}
        <div className="bg-muted rounded-2xl p-8 mb-16">
          <h3 className="text-2xl text-primary mb-6">
            Прогноз доходів та прибутку (тис. USD)
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={revenueData}>
              <defs>
                <linearGradient id="colorRevenue" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#1A6DCC" stopOpacity={0.8} />
                  <stop offset="95%" stopColor="#1A6DCC" stopOpacity={0} />
                </linearGradient>
                <linearGradient id="colorProfit" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#144073" stopOpacity={0.8} />
                  <stop offset="95%" stopColor="#144073" stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="year" />
              <YAxis />
              <Tooltip />
              <Legend />
              <Area
                type="monotone"
                dataKey="revenue"
                stroke="#1A6DCC"
                fillOpacity={1}
                fill="url(#colorRevenue)"
                name="Дохід"
              />
              <Area
                type="monotone"
                dataKey="profit"
                stroke="#144073"
                fillOpacity={1}
                fill="url(#colorProfit)"
                name="Прибуток"
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {/* Cost Structure Pie Chart */}
          <div className="bg-muted rounded-2xl p-8">
            <h3 className="text-2xl text-primary mb-6">Структура витрат</h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={costBreakdown}
                  cx="50%"
                  cy="50%"
                  labelLine={false}
                  label={({
                    name,
                    percent,
                  }: {
                    name: string;
                    percent: number;
                  }) => `${name} ${(percent * 100).toFixed(0)}%`}
                  outerRadius={100}
                  fill="#8884d8"
                  dataKey="value"
                >
                  {costBreakdown.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.color} />
                  ))}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
          </div>

          {/* Unit Economics */}
          <div className="bg-muted rounded-2xl p-8">
            <h3 className="text-2xl text-primary mb-6">Unit Economics</h3>
            <div className="space-y-4">
              {unitEconomics.map((item, index) => (
                <div
                  key={index}
                  className="flex justify-between items-center py-2 border-b border-border/50"
                >
                  <span className="text-foreground/80">{item.metric}</span>
                  <span className="text-primary">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* Investment Ask */}
        <div className="bg-gradient-to-br from-primary to-secondary rounded-2xl p-8 md:p-12 text-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-3xl mb-4">Інвестиційна пропозиція</h3>
              <p className="text-xl text-white/90 mb-6">
                Залучаємо $500K для масштабування виробництва та розширення
                команди продажів
              </p>
              <div className="space-y-3">
                <div className="flex items-center">
                  <div className="w-2 h-2 bg-white rounded-full mr-3" />
                  <span>Розширення виробничих потужностей</span>
                </div>
                <div className="flex items-center">
                  <div className="w-2 h-2 bg-white rounded-full mr-3" />
                  <span>Збільшення команди продажів до 10 осіб</span>
                </div>
                <div className="flex items-center">
                  <div className="w-2 h-2 bg-white rounded-full mr-3" />
                  <span>Розширення маркетингових каналів</span>
                </div>
                <div className="flex items-center">
                  <div className="w-2 h-2 bg-white rounded-full mr-3" />
                  <span>R&D нової лінійки продуктів</span>
                </div>
              </div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20">
              <div className="space-y-6">
                <div>
                  <div className="text-white/80 mb-2">Використання коштів</div>
                  <div className="space-y-2">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        <span>Виробництво</span>
                        <span>40%</span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-2">
                        <div
                          className="bg-white h-full rounded-full"
                          style={{ width: "40%" }}
                        />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        <span>Продажі & Маркетинг</span>
                        <span>35%</span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-2">
                        <div
                          className="bg-white h-full rounded-full"
                          style={{ width: "35%" }}
                        />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        <span>R&D</span>
                        <span>15%</span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-2">
                        <div
                          className="bg-white h-full rounded-full"
                          style={{ width: "15%" }}
                        />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        <span>Операційні витрати</span>
                        <span>10%</span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-2">
                        <div
                          className="bg-white h-full rounded-full"
                          style={{ width: "10%" }}
                        />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pt-4 border-t border-white/20">
                  <div className="text-white/80 mb-2">Очікуваний ROI</div>
                  <div className="text-4xl">3-5x</div>
                  <div className="text-white/80 text-sm">
                    протягом 3-4 років
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
